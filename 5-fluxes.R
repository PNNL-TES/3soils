...
sdata %>% 
  ...
  mutate(sm_gravimetric = (Total_core_mass_pot_pie_pans_g - DryMass_NONsoil_ALL_g) / DryMass_SoilOnly_g,
         sm_volumetric = (Total_core_mass_pot_pie_pans_g - DryMass_NONsoil_ALL_g - Additional_Wt_toRemove) / VolumeSoil_cm3) -> 
  sdata
...
sdata %>% 
  ...
  summarise(sm_gravimetric = mean(sm_gravimetric, na.rm = TRUE),
            sm_volumetric = mean(sm_volumetric, na.rm = TRUE)) %>% 
  print(n = 50)
...
fluxdata %>%
  ...
  group_by(TREATMENT_PHASE, Site, SampleID) %>% 
  ...
  group_by(TREATMENT_PHASE, Site, inctime_hours) %>% 
...
fd1 <- filter(fluxdata_by_site,
              TREATMENT_PHASE %in% c("DROUGHT_PRE_INCUBATION", "DROUGHT_INCUBATION", "FIELD_MOIST_INCUBATION", "SATURATION_INCUBATION"))
...
p_site_co2 <- ggplot(fd1, aes(inctime_hours, cumCO2_flux_mgC_gC, color = Site)) +
  ...
  facet_wrap(~TREATMENT_PHASE, scales = "free") + xlab("Incubation (hrs)") +
